...
def date_cleanse(path):
    try:
        for txt_file in os.listdir(path):
            print('Reformatting ' + txt_file + '...')
            f = open(path + '/' + txt_file, "r") # open the text file
            ...
            with open('output_data/new' + txt_file, 'w') as export:
                writer = csv.writer(export)
                print('Saving the reformatted date values...')
                for i in tqdm(formatted, leave=False): # for every entry in the formatted array
                    row = [i]
        ...
        print('\nFile directory not found. Please check the file path and try again.')
    except NameError:
        print('\nDependencies not met. Please check to ensure the datetime and tqdm modules are installed.')

date_cleanse(directory_path)
